		        ...
		        <ul class="layui-nav layui-layout-right">
		            <li class="layui-nav-item">
		                <a href="javascript:;">
		                    ...
		                    <img id="headImg" style="width: 40px;height: 40px" :src="'/app/download/downloadImg.do?imgUrl='+imgUrl">
		                </a>
...
<script>
	export default{
		...
		methods:{
			...
			getAccounts(){
				/* 路由 */
				this.$router.push({path:"/getAccounts",query:{rand:Math.random()}})
			},
			editHeadImg(){
				this.$router.push({path:"/editHeadImg",query:{rand:Math.random()}})
			},
			editPwd(){
			...
			getSearchs(){
				this.$router.push({path:"/getSearchs",query:{rand:Math.random()}})
			},
			getShips(){
